...
// Enhanced Cashier Analytics Component with Proper Credit Support
const CashierAnalytics = ({ cashier, transactions, credits, loading }) => {
  const [dateRange, setDateRange] = useState([
    dayjs().subtract(7, 'days'),
    dayjs()
  ...
  // Calculate cashier performance metrics with proper credit support
  const cashierStats = useMemo(() => {
    if (!cashier || !transactions || !Array.isArray(transactions)) {
      return getDefaultCashierStats();
    }
    ...
    // FIXED: Enhanced credit-specific calculations with proper filtering
    const cashierCredits = credits?.filter(credit => {
      if (!credit) return false;
      
      // Multiple ways to identify cashier association
      const creditCashierId = credit.cashierId || credit.cashier?._id || credit.createdBy?._id;
      const creditCashierName = credit.cashierName || credit.cashier?.name || credit.createdBy?.name;
      
      return creditCashierId === cashier._id || 
             creditCashierName === cashier.name;
    }) || [];
    
    const creditSales = filteredTransactions.filter(t => t.paymentMethod === 'credit');
    const totalCreditAmount = creditSales.reduce((sum, t) => sum + (t.totalAmount || 0), 0);
    
    // FIXED: Proper outstanding credit calculation
    const outstandingCredit = cashierCredits
      .filter(credit => {
        const status = credit.status || CalculationUtils.calculateEnhancedCreditStatus(credit);
    ...
    console.log('💰 Cashier Credit Analysis:', {
      cashierName: cashier.name,
      cashierId: cashier._id,
      creditsCount: credits?.length || 0,
      cashierCreditsCount: cashierCredits.length,
      outstandingCredit,
      creditSalesCount: creditSales.length,
      totalCreditAmount
    });
...
// Enhanced Cashier Management Component with Proper Credit Fetching
const CashierManagement = () => {
  const [cashiers, setCashiers] = useState([]);
  const [isModalVisible, setIsModalVisible] = useState(false);
  const [isViewModalVisible, setIsViewModalVisible] = useState(false);
  ...
  const createApiInstance = () => {
    const instance = axios.create({
      baseURL: process.env.REACT_APP_API_URL || 'https://back-pos-five.vercel.app',
      timeout: 10000,
      headers: {
        'Content-Type': 'application/json'
      }
  ...
  // FIXED: Enhanced fetchCashiers function
  const fetchCashiers = useCallback(async (forceRefresh = false) => {
    try {
      const cacheValid = cashiersCache.current.lastFetch && 
                       (Date.now() - cashiersCache.current.lastFetch < 30000);
  ...
  // FIXED: Enhanced fetchTransactions
  const fetchTransactions = useCallback(async (cashierId = null) => {
    try {
      setLoading(prev => ({ ...prev, analytics: true }));
      console.log('🔄 Fetching transactions for analytics...', { cashierId });
  ...
  // COMPLETELY REWRITTEN: Enhanced fetchCredits with multiple strategies like your working component
  const fetchCredits = useCallback(async (cashierId = null) => {
    try {
      console.log('🔄 Fetching credits data...', { cashierId });
      ...
      // Strategy 1: Try unified API first
      try {
        console.log('🔄 Strategy 1: Trying unified API...');
        const creditsData = await unifiedAPI.getCombinedCreditAnalysis(params);
        ...
        console.log('✅ Unified API credits:', creditsArray.length);
      } catch (unifiedError) {
        console.log('❌ Unified API failed, trying Strategy 2...', unifiedError);
        
        // Strategy 2: Try direct credit API via axios
        try {
          console.log('🔄 Strategy 2: Trying direct credit API...');
          const api = createApiInstance();
          const response = await api.get('/api/credits', { params });
          
          if (response.data && Array.isArray(response.data)) {
            creditsArray = response.data;
          } else if (response.data && Array.isArray(response.data.data)) {
            creditsArray = response.data.data;
          }
          
          console.log('✅ Direct credit API credits:', creditsArray.length);
        } catch (directError) {
          console.log('❌ Direct credit API failed, trying Strategy 3...', directError);
          
          // Strategy 3: Try with different endpoint
          try {
            console.log('🔄 Strategy 3: Trying alternative endpoint...');
            const api = createApiInstance();
            const response = await api.get('/api/credit-transactions', { params });
            ...
            console.log('✅ Alternative endpoint credits:', creditsArray.length);
          } catch (altError) {
            console.log('❌ All credit fetch strategies failed:', altError);
            throw new Error('All credit fetch strategies failed');
          }
        }
      }

      // FIXED: Enhanced credit processing with proper field mapping
      const processedCredits = creditsArray.map(credit => {
        // Extract fields from different possible structures
        const totalAmount = CalculationUtils.safeNumber(
          credit.totalAmount || credit.amount || credit.transactionAmount || 0
        );
        const amountPaid = CalculationUtils.safeNumber(
          credit.amountPaid || credit.paidAmount || credit.paid || 0
        );
        const balanceDue = CalculationUtils.safeNumber(
          credit.balanceDue || credit.outstandingAmount || credit.balance || 0
        );
        
        // Calculate balanceDue if not provided
        const calculatedBalanceDue = balanceDue > 0 ? balanceDue : Math.max(0, totalAmount - amountPaid);
        
        // Enhanced cashier identification
        const cashierId = credit.cashierId || credit.cashier?._id || credit.createdBy?._id;
        const cashierName = credit.cashierName || credit.cashier?.name || credit.createdBy?.name;
        ...
        return {
          ...
          status: credit.status || CalculationUtils.calculateEnhancedCreditStatus({
            ...credit,
            balanceDue: calculatedBalanceDue,
            amountPaid,
            totalAmount
      ...
      console.log('💰 FINAL Processed Credits:', {
        rawCount: creditsArray.length,
        processedCount: processedCredits.length,
        totalOutstanding: processedCredits.reduce((sum, credit) => sum + CalculationUtils.safeNumber(credit.balanceDue), 0),
        cashierCredits: cashierId ? processedCredits.filter(c => c.cashierId === cashierId).length : 'All'
      });

      setCredits(processedCredits);
      
    } catch (error) {
      console.error('💥 CRITICAL: All credit fetch strategies failed:', error);
      message.error('Unable to load credit data. Please check if credit services are running.');
      setCredits([]);
    }
  }, []);
  ...
  // FIXED: Enhanced analytics data fetching
  useEffect(() => {
    if (activeTab === 'performance' && selectedCashier) {
      console.log('📊 Loading analytics data for cashier:', selectedCashier.name, selectedCashier._id);
  ...
  // FIXED: Enhanced analytics handler
  const handleViewAnalytics = (cashier) => {
    setSelectedCashier(cashier);
    setActiveTab('performance');
